...
func (r *Repository) GetOrdersFiltered(ctx context.Context, status, start, end string) ([]ds.EmissionResponse, error) {
	...
	query = query.Where("emissions.status NOT IN ?", []string{"удален"})
...
// Сначала проверьте какие столбцы есть
func (r *Repository) UpdateDeviceEmissionCalculationResults(ctx context.Context, orderID int, totalEmission float64) error {
	fmt.Printf("\n=== [Repository] Обновление заказа %d ===\n", orderID)

	totalEmission *= 100000
	...
	fmt.Printf("Столбцы таблицы emissions:\n")
	for _, col := range columns {
		fmt.Printf("  %s: %s\n", col.ColumnName, col.DataType)
	}

	// Формируем запрос только с существующими столбцами
	updateFields := []string{"total_emission = ?", "status = 'завершен'"}
	...
	// Проверяем есть ли finish_at
	hasFinishAt := false
	for _, col := range columns {
		if col.ColumnName == "finish_at" {
			hasFinishAt = true
			break
		}
	}
	...
	if hasFinishAt {
		updateFields = append(updateFields, "finish_at = NOW()")
	}
	...
	fmt.Printf("Параметры: totalEmission=%f, orderID=%d\n", totalEmission, orderID)

	// Выполняем
	result := r.db.Exec(sql, totalEmission, orderID)
	fmt.Printf("Результат: RowsAffected=%d, Error=%v\n",
...
func (r *Repository) GetOrdersFilteredForUser(ctx context.Context, status, start, end string, userID int) ([]ds.EmissionResponse, error) {
	...
	if status != "" {
		...
		query = query.Where("emissions.status IN ?", []string{"сформирован", "завершен", "отклонен", "удален"})
	}
