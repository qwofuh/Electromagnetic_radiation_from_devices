...
func (h *Handler) RegisterHandler(router *gin.Engine) {
	...
	auth.Use(h.WithAuthCheck(role.User, role.Admin))
	{
		...
		auth.PUT("/emissions_calculation/devices/:order_id/:material_id/custom_power", h.UpdateCustomPowerAPI)
		auth.POST("/emissions_calculation/delete/:id", h.DeleteDevicesOrderAPI)
		auth.DELETE("/emissions_calculation/:order_id/device/:material_id", h.DeleteDeviceFromOrderAPI)
	}
